...
export default function UploadMomentScreen() {
  ...
  const handlePickVideo = useCallback(async () => {
    console.log("User tapped pick video for moment upload");
  ...
  const handleAddPlace = useCallback(() => {
    ...
    if (!trimmed) return;
    console.log("User added place to moment:", trimmed);
  ...
  const handleRemovePlace = useCallback((placeId: string) => {
    console.log("User removed place from moment:", placeId);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    console.log("User tapped Post Moment - submitting moment upload");
    setSubmitting(true);
    try {
      ...
      console.log("Step 1: Uploading video to Supabase Storage bucket 'posts'");
      ...
      console.log("Posting moment to /api/moments:", payload);
      await authenticatedPost("/api/moments", payload);
      console.log("Moment posted successfully");
      Alert.alert("Posted!", "Your moment has been shared.", [
        { text: "OK", onPress: () => router.back() },
      ]);
    } catch (e: any) {
      console.error("Upload moment error:", e);
