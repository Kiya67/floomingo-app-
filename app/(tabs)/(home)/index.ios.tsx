...
export default function HomeScreen() {
  ...
  const fetchMoments = useCallback(
    async (isRefresh = false, cursor: string | null = null) => {
      ...
      console.log(
        "Home grid (iOS): fetching moments from API, isRefresh:",
        isRefresh,
        "cursor:",
        cursor
      );
      try {
        ...
        console.log("Home grid (iOS): fetched", moments.length, "moments, next_cursor:", newNextCursor);
        ...
        console.error("Home grid (iOS): fetch moments error:", e);
  ...
  const fetchMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    console.log("Home grid (iOS): loading more moments, cursor:", nextCursor);
    await fetchMoments(false, nextCursor);
  }, [loadingMore, hasMore, nextCursor, fetchMoments]);

  useFocusEffect(
    useCallback(() => {
      console.log("HomeScreen (iOS) focused - fetching moments grid from API");
      setLoading(true);
      setNextCursor(null);
      setHasMore(true);
  ...
  const handleRefresh = useCallback(() => {
    console.log("User pulled to refresh home grid (iOS)");
    setRefreshing(true);
    setNextCursor(null);
    setHasMore(true);
  ...
  const handleItemPress = useCallback(
    (moment: Moment) => {
      console.log("User tapped video grid item (iOS), moment id:", moment.id);
    ...
    return (
      <View style={[styles.center, { paddingTop: insets.top }]}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            console.log("User tapped retry on home grid (iOS)");
            setLoading(true);
            setNextCursor(null);
            fetchMoments(false, null);
          }}
    ...
    return (
      <View style={[styles.center, { paddingTop: insets.top }]}>
        ...
        <TouchableOpacity
          style={styles.retryBtn}
          onPress={() => {
            console.log("User tapped refresh on empty home grid (iOS)");
            setLoading(true);
            setNextCursor(null);
            fetchMoments(false, null);
          }}
