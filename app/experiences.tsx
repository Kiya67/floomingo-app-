...
function SkeletonCard({ thumbHeight }: { thumbHeight: number }) {
  ...
  return (
    <View style={skeletonStyles.card}>
      <Animated.View
        style={[skeletonStyles.thumb, { opacity, height: thumbHeight }]}
      />
  ...
  const fetchExperiences = useCallback(
    async (cursor?: string, isRefresh = false) => {
      ...
      try {
        ...
        const newItems = Array.isArray(data?.experiences)
          ? data.experiences
          : [];
        if (cursor && !isRefresh) {
      ...
      <SafeAreaView style={styles.container} edges={["bottom"]}>
        ...
        <View style={styles.errorState}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              console.log("User tapped retry on experiences screen");
              setLoading(true);
              fetchExperiences();
            }}
            activeOpacity={0.8}
