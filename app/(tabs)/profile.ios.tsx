...
export default function ProfileScreen() {
  ...
  const fetchExperiences = async () => {
    console.log('Fetching user experiences (iOS)');
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;
      const { data, error } = await supabase
        ...
        .order('created_at', { ascending: false });
      if (error) {
        console.error('Error fetching experiences (iOS):', error);
      } else {
        console.log('User experiences fetched (iOS):', data?.length || 0);
        setExperiences(data || []);
      }
    } catch (error) {
      console.error('Error in fetchExperiences (iOS):', error);
    }
  };
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('Profile screen (iOS) focused - refreshing moments, experiences and stats');
        ...
        <View style={styles.postsSection}>
          ...
          <View style={styles.tabSwitcher}>
            <TouchableOpacity
              ...
              onPress={() => {
                console.log('User tapped Moments tab (iOS)');
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                console.log('User tapped Experiences tab (iOS)');
                ...
                {experiences.map((exp) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        console.log('User tapped experience (iOS):', exp.id);
                      ...
                      <View style={styles.expThumb}>
                        {expThumbSource ? (
                          ...
                          <View style={[styles.expThumbPlaceholder, { backgroundColor: primaryColor }]}>
                            <IconSymbol ios_icon_name="film" android_material_icon_name="movie" size={28} color="#fff" />
                          </View>
                        )}
                      </View>
