...
function ExperienceVideoPlayer({ videoUrl, experienceId }: { videoUrl: string; experienceId: string }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (isMountedRef.current) {
        try {
          player.play();
          console.log("Experience player (iOS): video started for:", experienceId);
        } catch (e) {
          console.error("Experience player (iOS): error starting video:", e);
        }
      }
    }, 400);
    return () => {
  ...
  const handleTap = useCallback(() => {
    console.log("User tapped experience video (iOS) - toggling play/pause:", experienceId);
    if (!player) return;
    try {
...
export default function ExperiencePlayerScreen() {
  ...
  useEffect(() => {
    if (!id) return;
    console.log("ExperiencePlayer (iOS): loading experience:", id);
    setLoading(true);
    Promise.all([
      ...
      .catch((e) => {
        console.error("ExperiencePlayer (iOS): load error:", e);
  ...
  const handleLike = useCallback(async () => {
    console.log("User tapped like on experience (iOS):", id);
    ...
    try {
      ...
      console.error("ExperiencePlayer (iOS): like error:", e);
      setIsLiked(wasLiked);
  ...
  const handleBookmark = useCallback(async () => {
    console.log("User tapped bookmark on experience (iOS):", id);
    if (!currentUserId || !id) return;
    ...
    try {
      ...
      console.error("ExperiencePlayer (iOS): bookmark error:", e);
      setIsBookmarked(wasBookmarked);
  ...
  const handleShare = useCallback(async () => {
    console.log("User tapped share on experience (iOS):", id);
    try {
      await Share.share({ message: experience?.title || "Check out this experience on Floomingo!" });
    } catch (e) {
      console.error("ExperiencePlayer (iOS): share error:", e);
    }
  }, [id, experience]);

  const handleProfilePress = useCallback(() => {
    if (!experience?.user_id) return;
    console.log("User tapped creator profile on experience (iOS):", experience.user_id);
    router.push(`/user/${experience.user_id}` as any);
  }, [experience, router]);

  const handlePlacePress = useCallback((place: Place) => {
    console.log("User tapped place pill on experience (iOS):", place.place_name);
    router.push(`/location/${place.place_id}` as any);
  }, [router]);

  const handleSuggestedPress = useCallback((exp: Experience) => {
    console.log("User tapped suggested experience (iOS):", exp.id, exp.title);
            ...
            <View style={styles.descSection}>
              ...
              {descLong ? (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    console.log("User tapped expand/collapse description (iOS)");
                    setDescExpanded((v) => !v);
                  }}
                  activeOpacity={0.7}
                >
