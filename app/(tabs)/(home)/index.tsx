...
import { Heart, Bookmark, Share2, MapPin, Film } from "lucide-react-native";
    ...
    if (isActive) {
      const t = setTimeout(() => {
        if (isMountedRef.current) {
          try {
            player.play();
            console.log("Home: video playing for moment:", momentId);
          } catch (e) {
            console.error("Home: error playing video:", e);
          }
        }
      }, 300);
...
export default function HomeScreen() {
  ...
  const fetchMoments = useCallback(async (cursor?: string) => {
    console.log("Home: fetching moments, cursor:", cursor);
    try {
  ...
  useFocusEffect(
    useCallback(() => {
      isMountedRef.current = true;
      console.log("HomeScreen focused - fetching moments");
      setLoading(true);
      fetchMoments();
      return () => {
        isMountedRef.current = false;
  ...
  const handleRefresh = useCallback(() => {
    console.log("User pulled to refresh moments feed");
    setRefreshing(true);
    setCurrentIndex(0);
    fetchMoments();
  }, [fetchMoments]);
  ...
  const handleLike = useCallback(
    async (moment: Moment) => {
      ...
      const cur = interactions.get(moment.id) ?? {
        ...
        bookmarks_count: moment.bookmarks_count,
      };
  ...
  const handleShare = useCallback(async (moment: Moment) => {
    ...
    try {
      await Share.share({
        message: moment.caption || "Check out this moment on Floomingo!",
  ...
  const handleExperiencePress = useCallback(
    (experienceId: string) => {
      console.log(
        "User tapped Watch Experience pill, experienceId:",
        experienceId
      );
  ...
  const handleBrowseExperiences = useCallback(() => {
    console.log("User tapped Browse Experiences button on home feed");
  ...
  const renderItem = useCallback(
    ({ item: moment, index }: { item: Moment; index: number }) => {
      ...
      const inter = interactions.get(moment.id) ?? {
        is_liked: moment.is_liked,
        is_bookmarked: moment.is_bookmarked,
        likes_count: moment.likes_count,
        bookmarks_count: moment.bookmarks_count,
      ...
      const linkedExpTitle =
        moment.linked_experience?.title || "Watch Experience";
      ...
      return (
        <View style={[styles.slide, { width, height }]}>
          ...
          <View style={styles.overlay}>
            {/* Experiences browse pill at top */}
            <View style={styles.topBar}>
              <TouchableOpacity
                style={styles.experiencesBrowsePill}
                onPress={handleBrowseExperiences}
                activeOpacity={0.85}
              >
                <Film size={13} color="#FFFFFF" strokeWidth={2} />
                <Text style={styles.experiencesBrowseText}>Experiences</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.bottomSection}>
              <View style={styles.leftContent}>
                <TouchableOpacity
                  ...
                  onPress={() => handleProfilePress(moment.user_id)}
                  activeOpacity={0.8}
                >
                  {avatarUrl ? (
                    <Image
                ...
                {hasLinkedExperience ? (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      handleExperiencePress(moment.linked_experience_id!)
                    }
                    activeOpacity={0.8}
                  >
                    <Film size={13} color="#FFFFFF" strokeWidth={2} />
              ...
              <View style={styles.rightActions}>
                <TouchableOpacity
                  ...
                  onPress={() => handleLike(moment)}
                  activeOpacity={0.8}
                >
                  <Heart
                    size={26}
                    color={likeColor}
                    fill={inter.is_liked ? PINK : "transparent"}
                    strokeWidth={2}
                  />
                ...
                <TouchableOpacity
                  style={styles.actionBtn}
                  onPress={() => handleBookmark(moment)}
                  activeOpacity={0.8}
                >
                  <Bookmark
                    size={26}
                    color={bookmarkColor}
                    fill={inter.is_bookmarked ? PINK : "transparent"}
                    strokeWidth={2}
                  />
    ...
    return (
      <View style={styles.center}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            console.log("User tapped retry on moments feed");
            setLoading(true);
            fetchMoments();
          }}
          activeOpacity={0.8}
  ...
  if (moments.length === 0) {
    return (
      <View style={styles.center}>
        ...
        <TouchableOpacity
          style={styles.experiencesBrowsePillLarge}
          onPress={handleBrowseExperiences}
          activeOpacity={0.85}
        >
          <Film size={15} color="#FFFFFF" strokeWidth={2} />
          <Text style={styles.experiencesBrowseTextLarge}>
            Browse Experiences
          </Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  experiencesBrowsePill: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
    backgroundColor: "rgba(255,59,122,0.85)",
    borderRadius: 20,
    paddingHorizontal: 14,
    paddingVertical: 8,
  },
  experiencesBrowseText: {
    fontSize: 13,
    color: "#FFF",
    fontWeight: "700",
  },
