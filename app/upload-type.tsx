...
export default function UploadScreen() {
  ...
  const handlePickVideo = useCallback(async () => {
    console.log("User tapped pick video on upload screen");
    ...
    if (!result.canceled && result.assets[0]) {
      ...
      console.log("User selected video:", asset.uri);
    }
  }, []);
  ...
  const handleAddPlace = useCallback(() => {
    ...
    if (!trimmed) return;
    console.log("User added place:", trimmed);
  ...
  const handleRemovePlace = useCallback((placeId: string) => {
    console.log("User removed place:", placeId);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    console.log("User tapped Post - submitting upload");
    setSubmitting(true);
    try {
      ...
      console.log("Posting to /api/moments:", payload);
  ...
  const handleClose = useCallback(() => {
    console.log("User tapped close on upload screen");
    router.back();
  }, [router]);
