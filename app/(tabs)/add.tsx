...
export default function AddScreen() {
  ...
  const uploadVideoAndThumbnail = async (userId: string) => {
    ...
    try {
      console.log("Trying bucket 'videos' for video upload");
      ...
      console.error("❌ 'videos' bucket failed:", uploadErr?.message || String(uploadErr), JSON.stringify(uploadErr));
      ...
      try {
        ...
        console.error("❌ Step 1 FAILED — both buckets failed:", fallbackErr?.message || String(fallbackErr), JSON.stringify(fallbackErr));
    ...
    try {
      ...
      try {
        console.log("Trying bucket 'videos' for thumbnail upload");
        ...
        console.warn("⚠️ 'videos' bucket failed for thumbnail, trying 'public':", thumbBucketErr?.message);
      ...
      console.warn("⚠️ Thumbnail generation/upload failed (continuing without thumbnail):", thumbErr?.message || String(thumbErr));
